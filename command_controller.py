...
class CommandController:
    ...
    def handle_command(self,command:CommandModel) -> None:
        ...
        if command.command_type == CommandType.TAKE_PHOTO.value:
            ...
            try:
                zoom_factor = float(command.command_value)
                self.set_zoom(zoom_factor)
            except (IndexError, ValueError):
                raise Exception("Invalid zoom command. Use 'zoom <factor>' where factor is a float.")
            ...
            try:
                bitrate = int(command.command_value)
                if not self.validator.validate_bitrate(bitrate):
                    raise Exception(f"Error: {bitrate} is not a valid bitrate. It must be between 500 and 10000 kbps.") 
                else:
                    print(f"Setting new bitrate: {bitrate} kbps")
                    bitrate = bitrate * 1000
                    self.reset_stream(("streaming_bitrate", bitrate))                         
            except (IndexError, ValueError):
                raise Exception("Invalid bitrate command. Use 'bitrate <value>' where value is an int 500-10000 kbps.")
        elif command.command_type ==CommandType.PORT.value:
            try:
                new_port = int(command.command_value)
                if not self.validator.validate_port(new_port):
                    raise Exception(f"Error: {new_port} is not a valid port. It must be between 1 and 65535.")                
                else:
                    print(f"Setting new port: {new_port}")
                    self.reset_stream(("destination_port", new_port))
            except (IndexError, ValueError):
                raise Exception("Invalid port command. Use 'port <value>' where value is an int between 1 and 65535.")          
        elif command.command_type==CommandType.IP_ADDRESS.value:
            try:
                new_iP = command.command_value
                ...
                raise Exception("Invalid ip command. Use 'ip <value>' where value is a valid ip address.")    
