...
class CommandModel(Model):
    ...
    command_type = CharField(
        index=True, choices=[(status.value, status.name) for status in CommandType]
    )
    command_value = CharField(null=True)
    command_status = CharField(
        index=True, choices=[(status.value, status.name) for status in CommandStatus]
    )
...
def get_pending_command() -> Optional[CommandModel]:
    ...
    with db.atomic():
        return (
            CommandModel.select()
            .where(CommandModel.command_status == CommandStatus.PENDING.value)
...
def update_command_status(command_id: str, new_status: CommandStatus) -> None:
    with db.atomic():
        query = CommandModel.update(command_status=new_status.value).where(
            CommandModel.id == command_id
        )
        query.execute()


def update_command_status_failure(
    command_id: str, meta_data: Optional[str] = None
) -> None:
    with db.atomic():
        query = CommandModel.update(
            command_status=CommandStatus.FAILED.value, meta_data=meta_data
