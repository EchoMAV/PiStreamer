...
def main(destination_ip, destination_port, bitrate, config_file):
    ...
    encoder = H264Encoder(
        bitrate=int(scaledBitrate), repeat=True, framerate=int(FRAMERATE)
    )
    ...
    if command == "stop":
        ...
        try:
            ...
            if not validate_port(newPort):
                print(
                    f"Error: {newPort} is not a valid port. It must be between 1 and 65535."
                )
            else:
...
def set_stream(picam2, ip, port, bitrate):
    ...
    picam2.stop()
    encoder = H264Encoder(
        bitrate=(int(bitrate) * 1000), repeat=True, framerate=int(FRAMERATE)
    )
    # output = FileOutput(FIFO_CAM_PATH)
...
if __name__ == "__main__":
    ...
    if not validate_port(args.port):
        print(
            f"Error: {args.port} is not a valid port number. It must be between 1 and 65535."
        )
        sys.exit(1)
    ...
    if not validate_bitrate(args.bitrate):
        print(
            f"Error: {args.bitrate} is not a valid bitrate. It must be between 500 and 10000 kbps."
        )
        sys.exit(1)
    ...
    if not is_json_file(args.config_file):
        print(
            f"Error: {args.config_file} is not a valid json file. Make sure the file exists."
        )
        sys.exit(1)
